...
const graphiteMod: BGExport = { Name: "gregtech:meta_block_compressed_12", Properties: { variant: "gregtech__graphite" } }
const berylliumMod: BGExport = { Name: "gregtech:meta_block_compressed_2", Properties: { variant: "gregtech__beryllium" } }
...
export const idToMapState: (((op: Options) => BGExport | undefined) | undefined)[] = [
    air, // 0: Air
    () => {return { Name: "nuclearcraft:cell_block" }} // 1: Reactor Cell
]

// Add Coolers and Active Coolers (IDs: 2-16, 34-48)
for (const cooler of coolerMap) {
    idToMapState[cooler[1]] = () => {return { Name: "nuclearcraft:cooler", Properties: { type: cooler[0].toLowerCase() } }};
    idToMapState[cooler[1] + 32] = (op: Options) => {
        if (op.activeCooler)
            return { Name: "nuclearcraft:active_cooler" };
        return undefined;
    };
}
